...
| F1-D2-b | Immutable decision-log entry with halt_reason for regulatory examination | `lip/c7_execution_agent/decision_log.py` | `9f623ce5` (Copilot initial C7 scaffold) | Copilot | Ryan (`a2b5d8a6` tenant_id threading); Claude `467b9a7d` added licensee_id field as AI-authored creative step not under contemporaneous direction — cleared to N via Condition 3 (post-AI rewrite at `d6b57305`). | `d6b57305` | `tomegah@...` | a, b, d | N |
| F1-D3 | Defense-in-depth: primary gate + secondary `_COMPLIANCE_HOLD_CODES` frozenset gate in agent.py | `lip/c7_execution_agent/agent.py`, `lip/c3_repayment_engine/rejection_taxonomy.py`, `lip/common/block_codes.json` | `c756cc4` (first BLOCK-set introduction, Ryan 2026-03-17) | `tomegah@...` (Ryan) | Ryan | `dd6f7806` (consolidate to shared JSON) | `tomegah@...` | a, d | N (fully human-authored; `770f70c8` is the EPG-19 expansion commit that added remaining BLOCK codes and BIC-first governing-law hook) |
...
| F4-I1 | Federated learning with DP-SGD + FedProx + Rényi DP accountant | `lip/p12_federated_learning/privacy_engine.py`, `dp_accountant.py`, `client.py`, `models.py` | `28afdd2a` (P12 PR #56) | `ryanktomegah@gmail.com` (Ryan) | Ryan | `28afdd2a` | `ryanktomegah@gmail.com` | a, e | N (fully human-authored via PR; Family 4 file never touched by any AI author) |
| F4-D2 | Layer partitioning — aggregate final layers only, keep counterparty-topology local | `lip/p12_federated_learning/models.py`, `lip/p12_federated_learning/local_ensemble.py` | `28afdd2a` | `ryanktomegah@gmail.com` (Ryan) | Ryan | `28afdd2a` | `ryanktomegah@gmail.com` | a, e | N |
| F4-D3 | FedProx proximal regularization μ in [0.001, 0.1] | `lip/p12_federated_learning/client.py`, `constants.py` | `28afdd2a` | `ryanktomegah@gmail.com` (Ryan) | Ryan | `28afdd2a` | `ryanktomegah@gmail.com` | a, e | N |
| F4-D4 | Phase 3 secure-aggregation upgrade (3-node threshold + semi-honest semantics) | `lip/p12_federated_learning/privacy_engine.py`, `docs/business/Capital-Partner-Strategy.md` (Phase 3 spec) | `28afdd2a` + `55275b2e` (capital strategy) | `ryanktomegah@gmail.com` / `noreply@anthropic.com` (Claude wrote Capital-Partner-Strategy doc, not code) | Ryan | `28afdd2a` | `ryanktomegah@gmail.com` | a, e | N (code is fully human-authored; Claude wrote the capital-strategy *doc* under user direction) |
| F4-D5 | Asynchronous quorum aggregation — tolerate non-responding nodes without failing round | `lip/p12_federated_learning/simulation.py` | `28afdd2a` | `ryanktomegah@gmail.com` (Ryan) | Ryan | `28afdd2a` | `ryanktomegah@gmail.com` | a, e | N |
| F4-D6 | Communication budget fits SWIFT SWIFTNet channel (no dedicated bandwidth) | `docs/business/Capital-Partner-Strategy.md`, `lip/p12_federated_learning/` | `28afdd2a` | `ryanktomegah@gmail.com` (Ryan) | Ryan | `28afdd2a` | `ryanktomegah@gmail.com` | a, e | N |
...
- **Family 1 (Taxonomy + Dual-Layer):** 8 elements mapped (I1-a, I1-b, D2-a, D2-b, D3, D3-norm, D4, D5). I1-a and F1-D3-norm are coded as one row each covering sub-bullets. **8 rows.**
- **Family 2 (Multi-Rail Settlement):** 10 elements (I1-a, I1-b, I1-c, I1-d, I1-e, D3, D4, D5, D6, D7). **10 rows.** *(Note: Claim 1 (System) and Claim 2 (Method) are both covered by the I1-a..e rows — §112 support and infringement-nexus analysis apply to both statutory classes from the same element set.)*
- **Family 3 (C4 Dispute + Human Override):** 9 elements (I1-a, I1-b, I1-c, I1-d, I1-e, D2, D3, D4, D5). **9 rows.**
...
- Condition 1 fails (AI did not create the creative step) — applies to 28 of 38 elements where the creative step was human-authored from inception;
- Condition 2 fails (paper-trail search returned positive human-direction evidence in at least one of locations a–e) — applies to all remaining 10 elements (F1-I1-a, F1-I1-b, F1-D2-b, F2-I1-a, F2-I1-e, F2-D4, F2-D7, F3-I1-a, F3-I1-b, F3-I1-c — where initial scaffolds were Copilot-authored but the claim-relevant creative logic was subsequently introduced or rewritten under documented EPG/GAP direction);
...
- **F1-I1-b (pipeline gate routing)** — scaffold by Copilot `cadf1905` (Phase 5 E2E); fall-through fix, hold-type routing, and compliance-hold block are Ryan's via `5564b40c`, `770f70c8`, `d5a1e28b` under EPG-09/10/19/27 direction.
- **F1-D2-b (decision_log.py)** — scaffold by Copilot `9f623ce5`; Ryan added `tenant_id` threading at `a2b5d8a6`. Claude added a `licensee_id` field at `467b9a7d` as an AI-authored creative step not under contemporaneous direction — cleared to N via Condition 3 (post-AI substantive rewrite at `d6b57305`).
...
- **F2-I1-e (repayment_loop.py, settlement_handlers.py)** — scaffold by Copilot `f729cb60`; partial-settlement policy and Redis idempotency-token preservation/consumption semantics were set by Ryan in `d4b593d4` (tenant-scoped Redis), `c10cd6c4` (GAP-05 royalty settlement), and `033fedf0` (taxonomy+settlement fail-closed).
- **F2-D4 (repayment_loop.py, settlement_handlers.py)** — scaffold by Copilot `f729cb60` (same commit as F2-I1-e); the Redis idempotency interface was scaffold-only, and the preserve-on-partial semantics + partial-settlement record dataclass fields are Ryan's. *Included by the same logic as F2-I1-e: same Copilot commit, same AI-scaffold-vs-Ryan-semantics split.*
- **F2-D7 (uetr_mapping.py, corridor_buffer.py)** — scaffold by Copilot `f729cb60` (same commit as F2-I1-e); the 30-min dedup window, 45-day TTL, and (bicA, bicB, amount, ccy) tuple key are Ryan's (per GAP-04 `d15e83ec`, finalised at `033fedf0`). *Included by the same logic as F2-I1-e: same Copilot commit, same AI-scaffold-vs-Ryan-semantics split.*
...
- **F3-I1-b (prefilter.py)** — scaffold by Copilot `463f6208`; the two-stage semantics (empty→UNKNOWN, keyword→hold-type-indicator) were set by Ryan in `3808a749`, `b7b09fd0`, `5aeae675`.
...
For the ten near-Y elements in §6.1 (F1-I1-a, F1-I1-b, F1-D2-b, F2-I1-a, F2-I1-e, F2-D4, F2-D7, F3-I1-a, F3-I1-b, F3-I1-c), counsel should request the following at the declaration-of-inventorship stage (35 U.S.C. § 115 declaration + AIA inventor oath):
...
6. **STOP condition #3 was considered and cleared.** Every claim element in `patent_claims_consolidated.md` has at least one implementation file in `lip/` — there are no "claim-only, no code" elements to flag.

7. **Thaler v. Vidal is evolving law.** The Federal Circuit's 2022 opinion forecloses AI-as-inventor but does not fully define "creative step" attribution thresholds when AI is used as a tool under human direction. Counsel should validate the three-condition test in §2.5 against current 2026 USPTO guidance before relying on this matrix as a filing-clearance artifact.
