...
| **What is needed** | Non-provisional patent filing based on `consolidation files/Provisional-Specification-v5.2.md`, with: (a) claims scoped to two-step classification + conditional offer logic per EPG-20; (b) language scrub per EPG-21 (no AML/SAR/OFAC/SDN/PEP/etc.); (c) BLOCK code list NOT enumerated; (d) clean-room re-implementation if any reviewer-provided text was incorporated. Pre-counsel risk analysis already complete in `docs/fundraising/ip-risk-pre-counsel-analysis.md` (140 KB). |
