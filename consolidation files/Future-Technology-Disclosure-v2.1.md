...
*Version 2.1 corrections: (A) All references to "Provisional Specification v5.0" updated to "v5.1" — appearing in the What Changed section, Extension G header, and Section G.5 (three occurrences). (B) Filing Trigger Table updated to replace absolute calendar years (2028, 2029, 2031, 2033, 2034) with PFD-relative offset language, consistent with Patent Family Architecture v2.1 offset convention. (C) Extension E.3 strategic priority note updated from absolute year range to PFD-relative offset (~Year 5–6). (D) Continuation Patents metadata field updated to reference PFD offset language. All technical content, extension descriptions, claim draft elements (a)–(ee), and strategic guidance unchanged.*
