...
class RepaymentConfirmation(BaseModel):
    ...
    platform_royalty_usd: Decimal = Field(
        default=Decimal("0"),
        ge=Decimal("0"),
        description="BPI technology licensor royalty (PLATFORM_ROYALTY_RATE × fee_repaid_usd).",
    )
...
class LoanOfferDelivery(BaseModel):
    ...
    Generated by OfferDeliveryService (lip.c7_execution_agent.offer_delivery)
    when C7 produces an OFFER decision. The ELO bank's treasury system receives
    this via webhook or Kafka event and has until offer_expiry (15 minutes) to
    respond with LoanOfferAcceptance or LoanOfferRejection.

    Resolves GAP-01: the missing offer delivery protocol.
    """

    model_config = ConfigDict(frozen=True)

    delivery_id: UUID = Field(..., description="Unique identifier for this delivery attempt.")
    offer_id: UUID = Field(..., description="LoanOffer.offer_id being delivered.")
    uetr: UUID = Field(..., description="ISO 20022 UETR of the underlying payment.")
    elo_entity_id: str = Field(
        ...,
        description="Hashed ELO entity identifier — the bank-side recipient of this offer.",
    )
    principal_usd: Decimal = Field(
        ...,
        gt=Decimal("0"),
    ...
    fee_amount_usd: Decimal = Field(
        ...,
        ge=Decimal("0"),
        description="Pre-computed fee: principal * (fee_bps/10000) * (maturity_days/365).",
    )
    maturity_days: int = Field(..., ge=1, description="Loan maturity window in calendar days.")
    rejection_code_class: str = Field(
        ...,
        pattern=r"^[ABC]$",
        description="Rejection-code class determining maturity_days (A=3, B=7, C=21).",
    )
    offer_expiry: datetime = Field(
        ...,
        description="UTC datetime after which this offer lapses with no acceptance.",
    )
    pd_score: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Probability-of-default from C2 at time of offer generation.",
    )
    delivery_endpoint: str | None = Field(
        default=None,
        description=(
            "Optional URL where the ELO treasury system should POST its "
            "LoanOfferAcceptance or LoanOfferRejection callback."
        ),
    )
...
class LoanOfferAcceptance(BaseModel):
    """ELO acceptance callback confirming the bank will fund the bridge loan.

    Sent by the ELO's treasury system in response to a LoanOfferDelivery.
    Receipt triggers C3 loan registration and bridge disbursement.
    The elo_operator_id satisfies EU AI Act Article 14 audit trail requirements.
    """

    model_config = ConfigDict(frozen=True)

    acceptance_id: UUID = Field(..., description="Unique identifier for this acceptance record.")
    delivery_id: UUID = Field(
        ...,
        description="LoanOfferDelivery.delivery_id this acceptance resolves.",
    )
    offer_id: UUID = Field(
        ...,
        description="LoanOffer.offer_id being accepted; echoed for downstream C3 registration.",
    )
    uetr: UUID = Field(..., description="ISO 20022 UETR of the underlying payment.")
    elo_entity_id: str = Field(..., description="Hashed ELO entity identifier.")
    elo_operator_id: str = Field(
        ...,
        min_length=1,
        description=(
            "Identifier of the treasury operator who accepted the offer. "
            "Required for EU AI Act Art.14 human-oversight audit trail."
        ),
    )
    accepted_at: datetime = Field(..., description="UTC timestamp of acceptance.")


class LoanOfferRejection(BaseModel):
    """ELO rejection callback: the bank declines to fund the bridge loan.

    The underlying payment fails normally; no bridge disbursement occurs.
    Rejection reason is retained for audit and operational analysis.
    """

    model_config = ConfigDict(frozen=True)

    rejection_id: UUID = Field(..., description="Unique identifier for this rejection record.")
    delivery_id: UUID = Field(
        ...,
        description="LoanOfferDelivery.delivery_id this rejection resolves.",
    )
    offer_id: UUID = Field(..., description="LoanOffer.offer_id being rejected.")
    uetr: UUID = Field(..., description="ISO 20022 UETR of the underlying payment.")
    elo_entity_id: str = Field(..., description="Hashed ELO entity identifier.")
    elo_operator_id: str = Field(
        ...,
        min_length=1,
        description="Identifier of the operator who rejected the offer.",
    )
