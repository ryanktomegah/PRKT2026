...
# VERSION 5.2 — PROSECUTION-READY

*System and Method for Automated Liquidity Bridging Triggered by Real-Time Payment Network Failure Detection*

| **Field** | **Value** |
|-----------|-----------|
| Document Version | 5.2 — Prosecution-Ready (§101 Re-Anchor) |
| Supersedes | v5.1 — Recentive Compliance Update |
...
One targeted correction to the Alice compliance analysis in Section 4.1.

Version 5.1 cited *Recentive Analytics, LLC v. Fox Corp.*, 52 F.4th 1357 (Fed. Cir. 2025) in support of patent eligibility, arguing that the claims here satisfy the standard Recentive articulated. This was a drafting error. *Recentive* is adverse authority: the Federal Circuit in that case held the claims *patent-ineligible* under § 101, finding that applying established machine learning techniques to a new data environment does not constitute a patent-eligible inventive concept absent a demonstrable improvement to the technical operation of the system itself. Citing a case where the claims failed as authority for claims that should succeed creates an unnecessary prosecution vulnerability. An examiner reading Section 4.1 would note the adverse holding immediately.

Version 5.2 removes all *Recentive* citations. The §101 analysis is re-anchored exclusively on *Enfish, LLC v. Microsoft Corp.*, 822 F.3d 1327 (Fed. Cir. 2016) and *McRO, Inc. v. Bandai Namco Games Am. Inc.*, 837 F.3d 1299 (Fed. Cir. 2016) — both of which found claims patent-eligible and both of which provide direct, affirmative support for the claims here. The technical infrastructure improvement arguments that v5.1 attributed to *Recentive* (latency, interoperability, settlement visibility) are preserved in full and re-anchored under *Enfish* and *McRO*, where they are more naturally and safely placed. The three-part infrastructure improvement analysis (Section 4.1, second substantive paragraph) is expanded and strengthened.

All claims, prior art distinctions, dependent claims, prosecution strategy, and continuation planning are unchanged from v5.1.
...
> **(t)** establishing a programmatic monitoring relationship between a disbursed liquidity advance and a specific cross-border payment transaction identified by a unique transaction reference assigned by the originating payment network, wherein the advance was disbursed to a party experiencing a working capital deficit caused by a failure, delay, or rejection condition affecting the identified payment transaction, and wherein said unique transaction reference is the individual UETR or equivalent network-assigned identifier that uniquely identifies the specific in-flight payment across all processing nodes;
>
...
> **CLAIM 5 NOTE — v5.2:** Step (t) and step (x) have been updated to include "individual UETR" explicitly by name, consistent with claim language standards established across the LIP patent family. This ensures UETR appears verbatim in the repayment loop claim, closing a drafting gap that existed in v5.1. All other claim elements unchanged.

---

## 3. Dependent Claims — v5.2 Final
...
**END OF PROVISIONAL SPECIFICATION v5.2**

**Supersedes:** v5.1 (Recentive Compliance Update — citation reversed)
**Key change:** *Recentive Analytics v. Fox* removed. §101 anchored on *Enfish* + *McRO* only.
**Claims:** Unchanged from v5.1 except Claims 5(t), 5(x), D3, D8, D13 — individual UETR added verbatim.
**Next step:** Engage patent attorney; review v5.2 with attorney; file provisional; commence P1 utility drafting.
